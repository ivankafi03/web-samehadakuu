...
export async function generateMetadata({ params }: GenrePageProps) {
    ...
    return {
        title: `Nonton Anime Genre ${genreName} Sub Indo - Samehadakuu`,
        description: `Koleksi lengkap anime genre ${genreName} subtitle Indonesia terbaru dan terbaik.`
    };
}
        ...
        <div className="flex flex-col gap-12 pb-20 pt-24 min-h-screen">
            <main className="max-w-7xl mx-auto px-6 w-full flex flex-col gap-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
                <div className="flex flex-col gap-2">
                    ...
                    <p className="text-zinc-500 text-sm font-black uppercase tracking-[0.2em]">Eksplorasi Koleksi {genreName}</p>
                </div>
                ...
                {mappedData.length > 0 ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-32 bg-white/5 rounded-[2.5rem] border border-white/5">
                        <p className="text-zinc-500 font-black uppercase tracking-[0.3em]">Data Tidak Ditemukan</p>
                    </div>
                )}
            </main>
        </div>
